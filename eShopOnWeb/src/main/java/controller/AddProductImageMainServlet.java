...
@WebServlet(urlPatterns = { "/admin/addProductImageMain" })
@MultipartConfig
public class AddProductImageMainServlet extends HttpServlet {
	@Override
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			req.getRequestDispatcher("/views/admin/add-productImage.jsp").forward(req, resp);
		} catch (Exception e) {
			e.getMessage();
		}
// có thể sử dụng thẻ <input type="hidden"> để truyền id qua bên image-list xử lý
	}
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			int productId = Integer.parseInt(idString);
			Product product = new Product();
			ProductImage productImageExist = dao.getByProductId(productId);
			int isDefault = 1;
